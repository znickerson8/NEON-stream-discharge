######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D03 - FLNT
...
#' discharge rating curve for Flint River for water years 2016-
...
surveyPtsDF <- read.table("~/D03_FLNT_AIS_surveyPts_20180125.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="ROAD6"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="ROAD6"]

# Manually adds weir and channel points from ADCP data
leftChannel<-data.frame("LCHANNEL",18.39,45.31,991.4404,"Transect")
names(leftChannel)<-c("name","N","E","H","mapCode")
leftWeir<-data.frame("LWEIR",18.39,45.31,991.2404,"Transect")
names(leftWeir)<-c("name","N","E","H","mapCode")
rightWeir<-data.frame("RWEIR",18.39,128.97,991.2404,"Transect")
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="ROAD6"]
...
staffGaugeMeterMark <- 3.50
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_3.50_USE",staffGaugePoints$name)]  
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-30,150))
...
geo_controlType_in$rectangularWidthUnc[1] <- 10.0 #Higher tha normal uncertainty because used ADCP data rather than survey

#Entries for Control #2
geo_controlType_in$hydraulicControlType[2] <- "Rectangular Channel"
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "LCHANNEL"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "RWEIR"]
geo_controlType_in$rectangularWidth[2] <- geo_controlType_in$controlRight[2]-geo_controlType_in$controlLeft[2]
geo_controlType_in$rectangularWidthUnc[2] <- 10.0 #Higher tha normal uncertainty because used ADCP data rather than survey

#Entries for Control #3
geo_controlType_in$hydraulicControlType[3] <- "Rectangular Channel"
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "BANK_TOP"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "BANK"]
...
#Slope calculations
# No thalweg or wetted edge survey data
# Estimated based on common slope for region
...
geo_priorParameters_in$priorActivationStageUnc[1] <- 1 #Higher tha normal uncertainty because used ADCP data

geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "LCHANNEL"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 1 #Higher tha normal uncertainty because used ADCP data

geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "BANK6"]
geo_priorParameters_in$priorActivationStageUnc[3] <- 1 #Higher tha normal uncertainty because used ADCP data
