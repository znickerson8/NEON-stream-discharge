######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D16 - MART
...
#' discharge rating curve for Martha Creek for water years 2023-.
...
filepath <- "N:/Science/AQU/Controls/D16_MART_20230921"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D16_MART_surveyPts_20230921.csv"),
                          sep = ",",
                          header = TRUE,
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_XS1"]
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-4,18))
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS3"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_LBF"]
...
geo_controlType_in$channelSlope[2] <- 0.02    # Used previous years slope.
geo_controlType_in$channelSlopeUnc[2] <- 0.02  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.02     # Used previous years slope.
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS17"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS9"]
