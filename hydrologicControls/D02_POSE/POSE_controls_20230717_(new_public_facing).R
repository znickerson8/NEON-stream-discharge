...
#   Bobby Hensley (2024-08-27)
#     Script for POSE 2023-07-13 created.
...
dataFromAPI <- neonUtilities::loadByProduct(streamMorphoDPID,siteID,startdate="2023-07", enddate="2023-07",
...
staffGaugeElevation <- staffGaugePoints$elevation[grepl("SP_0.60M",staffGaugePoints$surveyPointID)]  
...
geo_controlType_in$controlLeft[1] <- dsc_surveyPoints$distanceAdj[dsc_surveyPoints$surveyPointID == "DSC_XS5"]
geo_controlType_in$controlRight[1] <- dsc_surveyPoints$distanceAdj[dsc_surveyPoints$surveyPointID == "DSC_XS14"]
...
geo_controlType_in$channelSlopeUnc[2] <- 0.015 #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- rise/run
geo_controlType_in$channelSlopeUnc[3] <- 0.015 #Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[3] <- dsc_surveyPoints$gaugeHeight[dsc_surveyPoints$surveyPointID == "DSC_XS19"]
