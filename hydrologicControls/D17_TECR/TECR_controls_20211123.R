######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D17 - TECR
...
#' discharge rating curve for Teakettle Creek for water years 2022-.
...
filepath <- "N:/Science/AQU/Controls/D17_TECR_20211123"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D17_TECR_surveyPts_20211123.csv"),
                          sep = ",",
                          header = TRUE,
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_TEMP_PIN"]
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-12,12))
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_RFH"]
...
geo_controlType_in$channelSlopeUnc[2] <- 0.06  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- rise/run
geo_controlType_in$channelSlopeUnc[3] <- 0.06 # Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStageUnc[3] <- 0.3 # Combined uncertainty associated with survey and actual activation stage (0.1 m default, added a bit more for this one)
