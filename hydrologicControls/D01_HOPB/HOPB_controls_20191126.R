######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D01 - HOPB
...
#' discharge rating curve for Lower Hop Brook for water years 2017-201X.
...
surveyPtsDF <- read.table("~/D01_HOPB_surveyPts_20191126.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
names(surveyPtsDF) <- c("name","N","E","H","mapCode")
...
#Sets plot1 settings.  
xAxisTitle1<-list(title="Easting (m)",zeroline=FALSE, range=c(0,50))
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="RB_PIN"]
...
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_0.50M",staffGaugePoints$name)]  
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-25,25))
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-25,35))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS17"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS26"]
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS45"]
...
wettedEdgePoints=subset(surveyPtsDF,surveyPtsDF$mapCode%in%c("Slope"))
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS21"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS36"]
