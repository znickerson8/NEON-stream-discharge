######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D04 - GUIL
...
#' discharge rating curve for Rio Yahuecas for water years 2023-.
...
#   Bobby Hensley (10/24/2023)
#     Modified for GUIL 2023 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D04_GUIL_20231009"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D04_GUIL_surveyPts_20231009.csv"),
                          sep = ",",
                          header = TRUE,
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_PIN_LB"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_PIN_LB"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_PIN_LB"]
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-2,16))
...
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS20"]
...
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS20"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS20"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_RBF2"]
...
#Slope calculations
geo_controlType_in$channelSlope[2] <- 0.02 # No wetted edge points in survey. Used 2022 survey values.
geo_controlType_in$channelSlopeUnc[2] <- 0.02  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.02 # No wetted edge points in survey. Used 2022 survey values.
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS12"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_THL"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS20"]
