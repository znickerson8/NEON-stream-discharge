######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D15 - REDB
...
#' discharge rating curve for Red Butte Creek for water years 2022-.
...
filepath <- "N:/Science/AQU/Controls/D15_REDB_20221122"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D15_REDB_surveyPts_20221122.csv"),
                          sep = ",",
                          header = TRUE,
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-4,8))
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS31"]
...
#Slope calculations
geo_controlType_in$channelSlope[2] <- 0.05  #No wetted edge shots in survey. Used 2017 slope.
geo_controlType_in$channelSlopeUnc[2] <- 0.05  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.05 #No wetted edge shots in survey. Used 2017 slope.
geo_controlType_in$channelSlopeUnc[3] <- 0.05 # Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS19"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_RBF"]
