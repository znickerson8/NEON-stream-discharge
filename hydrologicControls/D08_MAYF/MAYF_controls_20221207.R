######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D08 - MAYF
...
#' discharge rating curve for Mayfield Creek for water years 2022-.
...
#   Bobby Hensley (10/25/2023)
#     Modified for MAYF 2022 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D08_MAYF_20221207"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D08_MAYF_surveyPts_20221207.csv"),
                          sep = ",",
                          header = TRUE,
                          stringsAsFactors = FALSE)

#Adds dummy point for wider flooplain control
floodplainPoint<-data.frame("DSC_RFPLN_5","","","","","","Transect_DSC",45,-10,998.8933)
names(floodplainPoint)<-colnames(surveyPtsDF)
surveyPtsDF <- rbind(surveyPtsDF, floodplainPoint)

#The end date of the geomorphology survey (YYYYMMDD)
surveyDate<-'20221207' 

#The date when this survey applies to the gauging record
surveyActiveDate <- "2021-10-01" #Start of WY22
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_LFPLN_1"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_LFPLN_1"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_LFPLN_1"]
...
gaugeName <- "SP_0.95"
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$name==gaugeName)
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-2,32))
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_LEW"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_REW"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS4"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_29"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_29"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_RFPLN_5"]
...
geo_controlType_in$channelSlope[2] <- 0.01  #Slope is too low to be determined from survey. Used default of 1%.
geo_controlType_in$channelSlopeUnc[2] <- 0.01  #Default slope uncertainty is equal to slope.
geo_controlType_in$channelSlope[3] <- 0.01 #Slope is too low to be determined from survey. Used default of 1%.
geo_controlType_in$channelSlopeUnc[3] <- 0.01 # Default slope uncertainty is equal to slope.
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_21"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_23"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_RFPLN_3"]
geo_priorParameters_in$priorActivationStageUnc[3] <- 0.2 # Combined uncertainty associated with survey and actual activation stage (0.1 m default, added a bit more for this one)
