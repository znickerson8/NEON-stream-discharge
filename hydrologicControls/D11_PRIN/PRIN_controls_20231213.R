...
#' discharge rating curve for Pringle Creek for water years 2023-.
...
#   Bobby Hensley (8/28/2024)
#     Modified for PRIN 2023 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D11_PRIN_20231213"

#Read in csv file
surveyPtsDF <- read.table(paste0(filepath,"/D11_PRIN_surveyPts_20231213.CSV"),
                          sep = ",",
                          header = TRUE,
...
surveyActiveDate <- "2022-10-01" #Start of WY23
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_LFPLN"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_LFPLN"]
...
staffGaugeElevation <- staffGaugePoints$H[grepl("S1_1.00M",staffGaugePoints$name)]  
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-30,8))
...
geo_controlInfo_in$controlActivationState[geo_controlInfo_in$controlNumber==1&geo_controlInfo_in$segmentNumber==4] <- 0
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_5"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_10"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_2"]
...
geo_controlType_in$controlLeft[4] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_LFPLN"]
geo_controlType_in$controlRight[4] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_2"]
...
#Slope calculations  No WE points in survey used THL
...
geo_controlType_in$channelSlopeUnc[3] <- 0.004 # Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[4] <- rise/run
geo_controlType_in$channelSlopeUnc[4] <- 0.004 # Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_6"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_10"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_14"]
...
geo_priorParameters_in$priorActivationStage[4] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_LFPLN4"]
