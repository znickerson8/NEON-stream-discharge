...
#' discharge rating curve for Caribou Creek for water years 2022-.
...
#   Bobby Hensley (12/01/2021)
#     Modified for OKSR 2021 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D19_CARI_20220901"

#Read in downloaded data
surveyPtsDF <- read.csv(file="D19_CARI_surveyPts_20220901.csv")

#The end date of the geomorphology survey (YYYYMMDD)
surveyDate<-'2022-09-01' 
...
floodplain<-data.frame("DSC_FLP","0","0","0","0","0","Transect_DSC","-20","38","997.6")
colnames(floodplain)<-colnames(surveyPtsDF)
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="DSC_LB_PIN_TOP"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS9"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 0.1

geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_RFH"]
