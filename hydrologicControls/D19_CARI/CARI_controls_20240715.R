######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D19 - CARI
...
#' discharge rating curve for Caribou Creek for water years 2024-.
...
#   Bobby Hensley (8/29/2024)
#     Modified for CARI 2024 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D19_CARI_20240715"

#Read in csv file
surveyPtsDF <- read.table(paste0(filepath,"/D19_CARI_surveyPts_20240715.CSV"),
                          sep = ",",
                          header = TRUE,
...
#Creates dummy floodplain point
floodplain<-data.frame("DSC_FLP","20","19.5","998.8","Transect_DSC")
colnames(floodplain)<-c("name","E","N","H","mapCode")
floodplain$E<-as.numeric(floodplain$E)
floodplain$N<-as.numeric(floodplain$N)
floodplain$H<-as.numeric(floodplain$H)
surveyPtsDF<-rbind(surveyPtsDF,floodplain)
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-2,12))
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS11"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS21"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_RBF"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_FLP"]
...
#Slope calculations - No wetted edge or thaweg shots in survey. Used previous slope.
geo_controlType_in$channelSlope[2] <- 0.006
geo_controlType_in$channelSlopeUnc[2] <- 0.006  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.006
geo_controlType_in$channelSlopeUnc[3] <- 0.006 # Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS16"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS18"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS26"]
