...
#' discharge rating curve for Caribou Creek for water years 2023-.
...
filepath <- "N:/Science/AQU/Controls/D19_CARI_20230719"

#Read in csv file
surveyPtsDF <- read.table(paste0(filepath,"/D19_CARI_surveyPts_20230719.CSV"),
                          sep = ",",
                          header = TRUE,
...
floodplain<-data.frame("DSC_FLP","-23","115","996.2","Transect_DSC")
...
staffGaugeMeterMark<-0.35
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_0.35M",staffGaugePoints$name)]  
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS12"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS14"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS22"]
