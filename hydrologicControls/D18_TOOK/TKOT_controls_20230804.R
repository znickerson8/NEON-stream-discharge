######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D18 - TKOT
...
#' discharge rating curve for Toolik Lake outlet for water years 2023-.
...
#   Bobby Hensley (4/23/2024)
#     Modified for TKOT 2023 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D18_TKOT_20230804"

surveyPtsDF <- read.table("~/D18_TKOT_surveyPts_20230804.csv",
                          sep = ",",
                          header = T,
                          stringsAsFactors = F,
                          encoding = "UTF-8")
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="OUT_DSC_XS37"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="OUT_DSC_XS37"]
...
dischargeXS1ReferenceDistance <- dischargePointsXS1$DistanceRaw[dischargePointsXS1$name=="OUT_DSC_XS37"]
...
#Adds floodplain point
floodPlain<-data.frame(name=c("OUT_DSC_FLD"),E=c(0.0),N=c(0.0),H=c(994.9),mapCode=c("Transect_DSC"),DistanceRaw=c(-10.0),DistanceAdj=c(-10.0))
dischargePointsXS1<-rbind(dischargePointsXS1,floodPlain)
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-11,31))
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_LEW"]
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_REW"]
...
geo_controlType_in$controlLeft[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_LBF"]
geo_controlType_in$controlRight[2] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_RBF"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_FLD"]
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "OUT_DSC_LBF"]
geo_controlType_in$rectangularWidth[3] <- geo_controlType_in$controlRight[3]-geo_controlType_in$controlLeft[3]
geo_controlType_in$rectangularWidthUnc[3] <- 5.0 #Extra uncertainty because not included in survey

#Slope calculations (No EW or THL shots)
geo_controlType_in$channelSlope[2] <- 0.01
geo_controlType_in$channelSlopeUnc[2] <- 0.01  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.01
...
geo_controlType_in$manningCoefficient[3] <- 0.1 # Willows and other brush 
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_XS28"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_XS24"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "OUT_DSC_XS34"]
