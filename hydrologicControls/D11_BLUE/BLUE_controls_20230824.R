...
#' discharge rating curve for Blue River for water years 2022-.
...
#   Bobby Hensley (10/23/2023)
#     Modified for BLUE 2023 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D11_BLUE_20230824"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D11_BLUE_surveyPts_20230824.CSV"),
                          sep = ",",
                          header = TRUE,
...
surveyID <- "BLUE_2023"  #These controls will be used for WY2020 rating curve 
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(0,45))
...
geo_controlType_in$controlRight[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS_16"]
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_RFPLN_12"]
...
#Slope calculations
geo_controlType_in$channelSlope[2] <- 0.005     # No wetted edge shots in 2023 survey. Used 2019 slope
geo_controlType_in$channelSlopeUnc[2] <- 0.005  # Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.005     # No wetted edge shots in 2023 survey. Used 2019 slope
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_10"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS_8"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 0.3 # Combined uncertainty associated with survey and actual activation stage (0.1 m default, added extra because staff gage not at discharge transect)

geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_REW"]
geo_priorParameters_in$priorActivationStageUnc[3] <- 0.5 # Combined uncertainty associated with survey and actual activation stage (0.1 m default, added extra because staff gage not at discharge transect)
