######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D07 - WALK
...
#' discharge rating curve for Walker Branch for water years 2022-.
...
#   Bobby Hensley (12/03/2023)
#     Modified for WALK 2017 survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D07_WALK_20220915"
...
surveyPtsDF <- read.table(paste0(filepath,"/D07_WALK_surveyPts_20220915.csv"),
                          sep = ",",
                          header = TRUE,
...
surveyActiveDate <- "2021-01-01" #Start of WY22
...
dischargeXS1NorthStart<-dischargePointsXS1$N[dischargePointsXS1$name=="DSC_LFH"]
dischargeXS1EastStart<-dischargePointsXS1$E[dischargePointsXS1$name=="DSC_LFH"]
...
staffGaugeMeterMark<-0.2
staffGaugeElevation <- staffGaugePoints$H[grepl(gaugeName,staffGaugePoints$name)]  
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS10"]
...
geo_controlType_in$controlLeft[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_LFH"]
...
# #Slope calculations
# No wetted edge or thalweg shots in survey. Using slope from previous survey.
geo_controlType_in$channelSlope[2] <- 0.015
geo_controlType_in$channelSlopeUnc[2] <- 0.013  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.015
geo_controlType_in$channelSlopeUnc[3] <- 0.013  #Default slope uncertainty is equal to slope
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS10"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS3"]
