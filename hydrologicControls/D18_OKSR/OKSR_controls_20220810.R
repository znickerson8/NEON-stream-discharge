...
#' discharge rating curve for Oksrukuyik Creek for water years 2022-
...
#   Bobby Hensley (12/20/2022)
#     Modified for OKSR 2022 AIS survey
######################################################################################################################## 
...
filepath <- "N:/Science/AQU/Controls/D18_OKSR_20220810"
URIpath <- paste(filepath,"filesToStack00131","stackedFiles",sep = "/")

# Read in survey points from csv file saved locally on N:/ drive
surveyPtsDF <- read.csv("N:/Science/AQU/Controls/D18_OKSR_20220810/D18_OKSR_surveyPts_20220810.csv")

#The end date of the geomorphology survey (YYYYMMDD)
surveyDate<-'2022-08-10' 
...
staffGaugeMeterMark<-1.10
staffGaugeElevation <- staffGaugePoints$H[grepl("SP_1.10M",staffGaugePoints$name)]  
...
geo_controlType_in$controlLeft[1] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS18"]
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_MAX_REW"]
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS18"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS23"]
