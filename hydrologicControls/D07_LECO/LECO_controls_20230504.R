...
filepath <- "N:/Science/AQU/Controls/D07_LECO_20230504"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D07_LECO_surveyPts_20230504.csv"),
                          sep = ",",
                          header = TRUE,
...
surveyActiveDate <- "2020-10-01" #Start of WY21
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-10,20))
...
#Slope calculations
geo_controlType_in$channelSlope[2] <- 0.02     # No wetted edge shots in 2023 survey. Used 2019 slope
geo_controlType_in$channelSlopeUnc[2] <- 0.02  # Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- 0.02     # No wetted edge shots in 2023 survey. Used 2019 slope
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS10"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS7"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS21"]
