...
staffGaugePoints=subset(surveyPtsDF,surveyPtsDF$name=="SP_0.65_B")
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-1,25))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
#Slope calculations
# No shots to use for slope in survey. using slope from 2018 survey
geo_controlType_in$channelSlope[2] <- 0.025
geo_controlType_in$channelSlopeUnc[2] <- 0.025
geo_controlType_in$channelSlope[3] <- 0.025
geo_controlType_in$channelSlopeUnc[3] <- 0.025
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS31"]
geo_priorParameters_in$priorActivationStageUnc[1] <- 0.1

geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS21"]
geo_priorParameters_in$priorActivationStageUnc[2] <- 0.1

geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS39"]
