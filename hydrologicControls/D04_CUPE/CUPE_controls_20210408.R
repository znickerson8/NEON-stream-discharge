...
#' discharge rating curve for Rio Cupeyes for water years 2022-.
...
filepath <- "N:/Science/AQU/Controls/D04_CUPE_20210408"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D04_CUPE_surveyPts_20210408.csv"),
                          sep = ",",
                          header = TRUE,
...
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(0,15))
yAxisTitle2<-list(title="Gauge Height  (m)",zeroline=FALSE)
font<-list(size=12,color='black')
plot_ly(data=dischargePointsXS1,x=~DistanceAdj, y=~gaugeHeight, name='Distance vs. Gauge Height', type='scatter', mode='markers+lines', text=~name)%>%
  add_trace(y= 0,name = 'Gauge Height = 0.00m',mode='lines',line = list(color = 'red', width = 2, dash='dash')) %>%
...
geo_controlType_in$controlRight[3] <- dischargePointsXS1$DistanceAdj[dischargePointsXS1$name == "DSC_XS8"]
...
geo_priorParameters_in$priorActivationStage[1] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS25"]
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS22"]
