...
filepath <- "N:/Science/AQU/Controls/D06_MCDI_20211103_WY22"
URIpath <- "N:/Science/AQU/Controls/D06_MCDI_20211103_WY22"

#Download data from API and store somewhere
dataFromAPI <- neonUtilities::zipsByProduct(streamMorphoDPID,siteID,startdate="2021-11", enddate="2021-11",package="expanded",check.size=FALSE,savepath = filepath)
...
surveyPtsDF <- read.table(paste0(URIpath,"/D06_MCDI_surveyPts_20211103.CSV"),
                          sep = ",",
                          header = TRUE,
...
geo_priorParameters_in$priorActivationStage[2] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_LC_LEW"]
...
geo_priorParameters_in$priorActivationStage[3] <- dischargePointsXS1$gaugeHeight[dischargePointsXS1$name == "DSC_XS41"]
geo_priorParameters_in$priorActivationStageUnc[3] <- 0.3 # Combined uncertainty associated with survey and actual activation stage (0.1 m default)
