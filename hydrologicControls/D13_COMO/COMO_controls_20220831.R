######################################################################################################################## 
#' @title Stage-Discharge Rating Curve Controls Script - D13 - COMO
...
#' discharge rating curve for Como Creek for water years 2022-.
...
filepath <- "N:/Science/AQU/Controls/D13_COMO_20220831"
...
surveyPtsDF <- read.table(paste0(URIpath,"/D13_COMO_surveyPts_20220831.csv"),
                          sep = ",",
                          header = TRUE,
...
#Sets plot2 settings.  
xAxisTitle2<-list(title="Distance (m)",zeroline=FALSE, range=c(-2,8))
...
geo_controlType_in$channelSlopeUnc[2] <- 0.08  #Default slope uncertainty is equal to slope
geo_controlType_in$channelSlope[3] <- rise/run
geo_controlType_in$channelSlopeUnc[3] <- 0.08 # Default slope uncertainty is equal to slope
